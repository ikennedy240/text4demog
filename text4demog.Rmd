...
cl_sample %>%
  # identify texts with particular keywords
  mutate(spacious = str_detect(cleanText, 'spacious'),
         storage = str_detect(cleanText, 'storage'),
         yard = str_detect(cleanText, 'yard'),
         big = str_detect(cleanText, 'big')) %>% 
  # group by the number of bedrooms
  group_by(cleanBeds) %>% 
  # summarrize across keyword vars
  summarise(across(spacious:big, mean)) %>%
  # pivot to a longer table for ggplot
  pivot_longer(cols = c(spacious, storage, yard, big), 
         names_to = 'keyword', 
         values_to = 'prevalance') %>%
  # plot results
  ggplot(aes(cleanBeds, prevalance, color = keyword, group = keyword))+
  geom_point()+
  facet_wrap(~keyword)
...
df <- df %>% mutate(
  ...
  text = str_replace_all(text,"(\\w+)\\W|/(\\w+)","\\1 \\2"),#  separate words joined by non-word characters
  text = str_remove_all(text, '\\d'), # get rid of numbers
  text = str_remove(text, 'https|t.co')) # more url cleaning
...
# now we'll fit a model looking at words associated with higher rent
...
sort(m0$coefficients, decreasing = TRUE)[1:40]
```
